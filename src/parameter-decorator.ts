  ...
  return function(target: any, name: string, index: number) {
    ...
    switch (name) {
      ...
      case 'instanceSayHi':
        ...
        assert(
          target.constructor.name === 'Greeter',
          'Target is the prototype of Greeter class'
        );
        assert(
          typeof target.constructor === 'function',
          'Target is the prototype of Greeter class, which is a function'
        );
        assert(
          target.constructor.prototype === target,
          "Target's constructor points to the Greeter class"
        );
        break;
    }
    // here, as part of the demonstration solution, parameter decorator registers the parameter test with the original method
    const parameterTestRegister = Object.getOwnPropertyDescriptor(
      originalMethod,
      parameterDecorationSymbol
    );
...
function processParameters(
  ...
  const parameterTestRegisterDescriptor = Object.getOwnPropertyDescriptor(
    originalMethod,
    parameterDecorationSymbol
  );
  if (parameterTestRegisterDescriptor) {
    const parameterTestRegister = parameterTestRegisterDescriptor.value as [
      number,
      (p: any) => any
    ][];
...
export class Greeter {
  @processParameters
  public static staticSayHi(
    @addExclamation who: any,
    @addExclamation day: string
  ): string {
    return 'Hi ' + who + ', happy ' + day;
  }
  @processParameters
  public instanceSayHi(
    @addExclamation who: any,
    @addExclamation day: string
  ): string {
