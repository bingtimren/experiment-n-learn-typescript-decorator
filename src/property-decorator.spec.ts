...
function staticPropertyDecorator(target: any, name: string) {
    ...
    callSequence++
    assert(callSequence > 1 && callSequence < 5)
...
function instancePropertyDecorator(target: any, name: string) {
    ...
    const decoratedPropertyNames = target.constructor[instancePropertyListForInjectionSymbol] as undefined|string[]
 ...
 */
function instancePropertyInjectionDecorator<T extends new (...args:any[])=>object>(target:T){
    ...
    return class ExtendedTarget extends target {
        constructor(...args:any[]){
            ...
            // do the injection
            const propertyList = (target as any)[instancePropertyListForInjectionSymbol] as undefined|string[]
            if (propertyList) {
...
test('Static property decorator', t => {
    ...
    t.is(ExtendedDecoratedGreeter.undefinedStaticProperty, "decorator initialized")
